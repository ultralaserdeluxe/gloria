...
\usepackage{fancyhdr}
\pagestyle{fancy}
\lhead{\includegraphics[scale=0.12]{../mall/Logga1.png}}
\rhead{\today}
...
\newcommand{\LIPSmilstolpe}[4][Krav \arabic{LIPSstolpnummer}]{{#1} & {#2} & {#3} \stepcounter{LIPSstolpnummer}\\\hline}

% Aktivitets-lista
\newenvironment{LIPSaktivitetslista}{
	\center
		\tabularx{\textwidth}{| p{0.3cm} | X |}
			\hline
			\textbf{Nr} & \textbf{Beskrivning} \\\hline
}
{
		\endtabularx
	\endcenter
...
\newcommand{\LIPSaktivitet}[4][\arabic{LIPSstolpnummer}]{{#1} & {#2} \stepcounter{LIPSstolpnummer}\\\hline}
